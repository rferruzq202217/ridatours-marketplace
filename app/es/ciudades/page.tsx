    ...
    <div className="min-h-screen bg-gray-50 pt-24">
      ...
      <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-xl text-blue-100 max-w-2xl">Echa un vistazo a nuestra lista de ciudades: desde lugares emblemáticos hasta experiencias inolvidables.</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {sortedCountries.map((country) => (
          <div key={country} className="mb-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
              <span>{country}</span>
              <span className="text-sm font-normal text-gray-500 bg-gray-100 px-3 py-1 rounded-full">{citiesByCountry[country].length} {citiesByCountry[country].length === 1 ? 'ciudad' : 'ciudades'}</span>
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
              {citiesByCountry[country].map((city) => (
                <Link key={city.slug} href={`/es/${city.slug}`} className="group relative overflow-hidden rounded-2xl aspect-[4/3] bg-gray-200">
                  {city.image && <Image src={city.image} alt={city.name} fill className="object-cover group-hover:scale-110 transition-transform duration-500" sizes="(max-width: 768px) 50vw, 20vw" />}
        ...
        {filteredCities.length === 0 && (
          <div className="text-center py-16">
            ...
            <Link href="/es/ciudades" className="inline-flex bg-blue-600 text-white px-6 py-3 rounded-full font-medium hover:bg-blue-700">Ver todas las ciudades</Link>
          </div>
        )}
      </div>
